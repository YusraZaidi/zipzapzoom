...
<!DOCTYPE html>
         ...
         <%
           ...
           if(utype.equals("Customer")){ps=con.prepareStatement("select oid from query_table where cust_email=? ");}
           else{
               ps=con.prepareStatement("select oid from query_table where trans_email=? ");
           }
            //PreparedStatement ps=con.prepareStatement("select oid from query_table where cust_email=?");
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav left_col">
       <%
        ...
        <li class="active"><a href="previousorder.jsp">All Orders</a></li>
  ...
  <div class="row content">
    <div class="sidenav col-sm-2  hidden-xs left_col  ">
      ...
      <ul class="nav nav-pills  nav-stacked ">
        ...
        <li class="list active"><a href="previousorder.jsp"><i class="fa fa-table" style="font-size:170%;color:white;" aria-hidden="true"></i>All Orders</a></li>
...
//                         Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/zipzapzoom","root","");
    ...
    String l_place="",l_date="",d_date="",d_place="",status="",str="";
//    int price=0;
    while(rs1.next()){
            ...
            str="AA Verification"; 
             ...
             str="Delivered";
              ...
              <tr>
                ...
                <td><%=d_place%></td>
                <td><%=d_date%></td>
                <%if(str.equals("Delivery Process")&&(utype.equals("Customer"))){%>
                <td><a href="orderinfo.jsp?oid=<%=oid%>"><%=str%></a></td>
                
                <%}else if(str.equals("Delivery Process")&&(utype.equals("Transporter"))){%>
                <td><a href="order_trans_info.jsp?oid=<%=oid%>"><%=str%></a></td>
                
                <%}else if(str.equals("Live Biding")){%>
                <td><a href="bidingoid.jsp?oid=<%=oid%>"><%=str%></a></td>
                <%}else{%><td><%=str%></td><%}%>
              </tr>
               <%}%>
