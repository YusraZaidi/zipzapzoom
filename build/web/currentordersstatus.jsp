...
<!DOCTYPE html>
         ...
         <%
           ...
           if(utype.equals("Customer")){ps=con.prepareStatement("select oid from query_table where cust_email=? and status=?");}
           else{
               ps=con.prepareStatement("select oid from query_table where trans_email=? and status=?");
           }
  ...
  <div class="row content">
    <div class="sidenav col-sm-2  hidden-xs left_col  ">
      ...
      <ul class="nav nav-pills  nav-stacked ">
        ...
        <li class="list active"><a href="currentordersstatus.jsp"><i class="fa fa-clock-o" style="font-size:170%;color:white;" aria-hidden="true"></i>Orders Ongoing</a></li>
          ...
          <table class="table table-bordered table-dark table-hover">
            <thead>
              <tr>
                ...
                <%
           if(utype.equals("Customer")){%>
                <th scope="col">Transporter</th><%}else{%>
...
//                         Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/zipzapzoom","root","");
    ...
    String l_place="",l_date="",d_date="",d_place="",transporter="";
    int price=0;
    while(rs1.next()){
        ...
        price=rs1.getInt(8);
        if(utype.equals("Customer")){
         transporter=rs1.getString(12);}else{
             transporter=rs1.getString(11);
        }
    }
                %>
              <tr>
